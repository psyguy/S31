...
output: 
  html_document:
    ...
    highlight: tango
    codefolding: hide
    pandocargs: --output=3B-Lab-Trees-answer.html
---
...
data(spam)
set.seed(10) 
train <- createDataPartition(y = spam$type, p = .75, list = FALSE)
```
...
```{r}
train_tree <- tree(type ~ ., data = spam[train, ])
plot(train_tree)
...
```{r}
prune_tree <- prune.tree(train_tree, best = 11)
```
...
```{r}
(train_bag <- randomForest(type ~ ., spam[train, ], importance = TRUE, mtry = 57))
```
...
```{r}
(train_rf  <- randomForest(type ~ ., spam[train, ], importance = TRUE))
```
...
```{r}
class_tree  <- predict(prune_tree,           spam[-train, ], type = "class")

class_bag   <- predict(train_bag,            spam[-train, ])

class_rf    <- predict(train_rf,             spam[-train, ])

class_boost <- predict(train_boost,          spam[-train, ])

class_svm   <- predict(train_svm$best.model, spam[-train, ])
```


```{r}
cf <- function(obs = spam[-train, 58], est)table(obs = obs, est = est)
