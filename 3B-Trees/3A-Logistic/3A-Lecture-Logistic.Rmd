...
The linear models is unsuited for this purpose, but what is? 
...
$$P(Y=\text{success})=\frac{e^{\beta_0+\beta_1x_1+\dots}}{1+e^{\beta_0+\beta_1x_1+\dots}}=\frac{1}{1+e^{-(\beta_0+\beta_1x_1+\dots)}}$$
...
Logistic model is _generalized linear model_ with link function


$$\text{logit}(Y)=\beta_0+\beta_1x_1+\dots+\beta_kx_k$$
$~$

The "logit" is the log of the odds, so that

$$\text{odds}(Y)=\frac{P(Y)}{1-P(Y)}=e^{\beta_0+\beta_1x_1+\dots+\beta_kx_k}$$
...
data.frame(
  probability    =            p[1:10],
  classification = as.numeric(p[1:10] > .5)
) %>% round(3) 
```
...
- unreliable parameter estimates when
    ...
    - factor interactions with (almost) empty cells 
...
Separates classes based on discriminant functions
...
knitr::include_graphics("pics/LDA.png")
```
...
1. Estimate _posterior_ probability $P(X=x|Y=j)$ of class $j=1,\dots,J$ 
...
- $\pi_j$ is _posterior probability_ class $j$ (estimated by sample proportion) 
...
- quadratic dicriminant functions
